...
# from .config import mb_ssd_512_config as config
...
def create_mobilenetv2_ssd_lite_512(num_classes, width_mult=1.0, use_batch_norm=True, onnx_compatible=False, is_test=False):
    ...
    regression_headers = ModuleList([
        SeperableConv2d(in_channels=round(576 * width_mult), out_channels=anchors[0]* 4,kernel_size=3, padding=1, onnx_compatible=False),
        SeperableConv2d(in_channels=1280, out_channels=anchors[1] * 4, kernel_size=3, padding=1, onnx_compatible=False),
        SeperableConv2d(in_channels=512, out_channels=anchors[2] * 4, kernel_size=3, padding=1, onnx_compatible=False),
        SeperableConv2d(in_channels=256, out_channels=anchors[3] * 4, kernel_size=3, padding=1, onnx_compatible=False),
        SeperableConv2d(in_channels=256, out_channels=anchors[4] * 4, kernel_size=3, padding=1, onnx_compatible=False),
...
def create_mobilenetv2_ssd_lite_predictor_512(net, candidate_size=200, nms_method=None, sigma=0.5, device=torch.device('cpu')):
    predictor = Predictor(net, config.image_size, config.image_mean,
                          config.image_std,
                          nms_method=nms_method,
                          iou_threshold=config.iou_threshold,
...
def create_mobilenetv2_ssd_lite_FPN38(num_classes, width_mult=1.0, use_batch_norm=True, onnx_compatible=False, is_test=False):
    anchors = [6,6,6,6]
    base_net = MobileNetV2(width_mult=width_mult, use_batch_norm=use_batch_norm,
                           onnx_compatible=onnx_compatible).features
...
def create_mobilenetv2_ssd_lite_predictor_FPN38(net, candidate_size=200, nms_method=None, sigma=0.5, device=torch.device('cpu')):
    predictor = Predictor(net, config.image_size, config.image_mean,
                          config.image_std,
                          nms_method=nms_method,
                          iou_threshold=config.iou_threshold,
...
def create_mobilenetv2_ssd_lite_224(num_classes, width_mult=1.0, use_batch_norm=True, onnx_compatible=False, is_test=False):
    anchors = [6,6,6,6,6,6]
    base_net = MobileNetV2(width_mult=width_mult, use_batch_norm=use_batch_norm,
                           onnx_compatible=onnx_compatible).features
    # print(base_net)
...
def create_mobilenetv2_ssd_lite_predictor_224(net, candidate_size=200, nms_method=None, sigma=0.5, device=torch.device('cpu')):
    predictor = Predictor(net, config.image_size, config.image_mean,
                          config.image_std,
                          nms_method=nms_method,
                          iou_threshold=config.iou_threshold,
